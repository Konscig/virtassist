...
Использует qwen3.5:35b-a3b для автоматической разметки вопросов/ответов.
"""
...
def call_ollama(
    ...
    try:
        logger.info(f"Calling Ollama, question: {question[:100]}...")
        response = ollama.chat(
            ...
            format=AnnotationResult.model_json_schema(),
        )
        logger.info(f"Ollama response received, parsing JSON...")
        content = response["message"]["content"]

        result = AnnotationResult.model_validate_json(content)
...
def annotate_dataset(
    ...
    for i, item in enumerate(items):
        ...
        if (i + 1) % batch_size == 0:
            ...
            total_duration = batch_end_time - total_start_time

            # Calculate speed
            batch_speed = batch_size / batch_duration
            avg_speed = (i + 1) / total_duration
            ...
            logger.info(
                f"📊 Batch speed: {batch_speed:.2f} questions/sec ({batch_speed * 60:.1f} questions/min)"
            )
            logger.info(
                f"📊 Average speed: {avg_speed:.2f} questions/sec ({avg_speed * 60:.1f} questions/min)"
            )
            logger.info(f"📊 Batch duration: {batch_duration:.1f}s")
            logger.info(f"📊 Total time: {total_duration / 60:.1f}min")
            logger.info(f"📊 Remaining: {remaining} questions")
            logger.info(f"📊 ETA: {eta_minutes:.1f}min ({eta_hours:.2f}h)")
    ...
    # Final statistics
    avg_speed = total / total_duration if total_duration > 0 else 0
    ...
    logger.info(f"📊 Total time: {total_minutes:.1f}min ({total_hours:.2f}h)")
    logger.info(
        f"📊 Average speed: {avg_speed:.2f} questions/sec ({avg_speed * 60:.1f} questions/min)"
    )
