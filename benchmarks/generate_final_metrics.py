#!/usr/bin/env python3
"""Скрипт для генерации финальных метрик и графиков."""
...
def create_final_metrics():
    ...
    metrics = {
        "dataset_info": {
            ...
            "description": "Полные метрики на основе аннотации всего датасета (1305 записей)",
        },
        "metrics": {
            "answer_types": {
                "без_ответа": {
                ...
                "некорректные": {
                    "count": 888,
                    "percent": 68.0,
                    "description": "Некорректные или неполные ответы",
                },
                "релевантные": {
                    "count": 133,
                    "percent": 10.2,
                    "description": "Полностью релевантные ответы",
                },
            },
            "question_types": {
                "по_делу": {
                    "count": 1066,
                    "percent": 81.7,
                    "description": "Конкретные вопросы по существу",
                },
                "small_talk": {
                    "count": 208,
                    "percent": 15.9,
                    "description": "Приветствия и социальные взаимодействия",
                },
                "про_вопрошалыча": {
                    "count": 31,
                    "percent": 2.4,
                    "description": "Вопросы о системе Вопрошалыч",
                },
            },
            "url_relevance": {
                "url_релевантен": {
                    "percent": 36.5,
                    "count": 476,
                    "description": "URL релевантен вопросу",
                },
                "ответ_использует_url": {
                    "percent": 30.6,
                    "count": 399,
                    "description": "Ответ использует информацию из URL",
                },
                "has_source": {
                    "percent": 61.8,
                    "count": 806,
                    "description": "Записи с указанным источником",
                },
                "without_source": {
                    "percent": 38.2,
                    "count": 499,
                    "description": "Записи без источника",
                },
            },
            "user_scores": {
                "total_with_score": 35,
            ...
            "empty_answers": {
                "count": 302,
                "percent": 23.1,
                "description": "Записи с пустым полем answer",
            },
        },
...
def plot_answer_types(metrics: dict, output_path: Path):
    ...
    answer_types = metrics["metrics"]["answer_types"]
    labels = [
...
def plot_question_types(metrics: dict, output_path: Path):
    ...
    question_types = metrics["metrics"]["question_types"]
    labels = [
...
def plot_source_relevance(metrics: dict, output_path: Path):
    ...
    url_rel = metrics["metrics"]["url_relevance"]
...
def plot_user_scores(metrics: dict, output_path: Path):
    ...
    user_scores = metrics["metrics"]["user_scores"]
...
def plot_rag_pipeline_analysis(metrics: dict, output_path: Path):
    ...
    percentages = [100.0, 61.8, 36.5, 30.6, 10.2]
...
def main():
    ...
    print("Генерация финальных метрик...")
    metrics = create_final_metrics()

    print("\nСохранение JSON отчета...")
    output_path = output_dir / "final_metrics_report.json"
    ...
    print("\nПостроение графиков...")
    plot_answer_types(metrics, output_dir / "answer_types_final_chart.png")
    plot_question_types(metrics, output_dir / "question_types_final_chart.png")
    plot_source_relevance(metrics, output_dir / "source_relevance_final_chart.png")
    plot_user_scores(metrics, output_dir / "user_scores_final_chart.png")
    plot_rag_pipeline_analysis(metrics, output_dir / "rag_pipeline_analysis_final.png")
