...
Внешние метрики используются для оценки проблем в работе чат-бота и демонстрации необходимости улучшений:

- **Анализ пользователей** (`benchmarks/analyze_users_domain.py`):
  - Всего пользователей
  - Пользователи без вопросов (%)
  - Пользователи с безответными вопросами (%)
  - Среднее число вопросов на пользователя
  - Распределение вопросов по пользователям
  - Timeline: вопросы и пользователи по дням

- **Анализ вопросов** (`benchmarks/analyze_real_users_domain.py`):
  - Всего вопросов
  - Вопросы с ответами / без ответов (%)
  - Распределение оценок пользователей
  - Топ токенов в вопросах

### Внутренние метрики (Internal Metrics)
Технические бенчмарки для глубокой оценки качества RAG-системы:
...
Вкладки дашборда:
- Run Details — детали запусков бенчмарков
- Metric History — история метрик
- Domain Insights — анализ вопросов
- User Analytics — анализ пользователей
- Vector Space — визуализация векторного пространства
- Chunk Utilization — использование чанков
- Topic Coverage — покрытие тем
